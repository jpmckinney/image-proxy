...
function retrieveRemote(remote, width, height, extention) {
  ...
  var agent = options.protocol === 'http:' ? http : https
    ...
    , request = agent.get(options, function (response) {
        ...
        if ((response.statusCode === 301 || response.statusCode === 302) && response.headers['location']) {
          ...
          return retrieveRemote(url.format(redirect), weight, height, extention);
        }
...
module.exports = function () {
  app.get('/:url', function (req, res, next) {
    ...
    retrieveRemote('http://s3.amazonaws.com/qmerce-static-media/' + req.params.url, null, null, null);
  });
  ...
  app.get('/:url/:width/:height.:extension?', function (req, res, next) {
    ...
    retrieveRemote('http://s3.amazonaws.com/qmerce-static-media/' + req.params.url, width, height, extension);
  });
