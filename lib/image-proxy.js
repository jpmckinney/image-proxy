...
var express = require('express'),
  ...
  ENABLE_GIF_COMPRESSION = 'false',
  CDN_ORIGIN = process.env.CDN_ORIGIN || `https://storage.googleapis.com/${process.env.NODE_ENV === 'production' ? 'apester-static' : 'apester-stg'}/`,
  delay = parseInt(process.env.DELAY) || 5000,
  mimeTypes = ['image/gif', 'image/jpeg', 'image/png', 'image/jpg', 'image/svg+xml'];

function retrieveRemote(res, next, remote, width, height, extension, filter) {
  console.log(`remote=${remote} width=${width} height=${height}`);
  ...
  var agent = options.protocol === 'http:' ? http : https,
    ...
    request = agent
      .get(options, function(response) {
        ...
        /**
         * resize gif, we apply this block if image type match and only if the requested
         * image has width param set not to 'undefined'
         */
        if (ENABLE_GIF_COMPRESSION && mimeType === mime.getType('gif') && (width !== 'undefined' || height !== 'undefined')) {
          const spawn = require('child_process').spawn;
          const args = [
            // resize by either width or height
            width !== 'undefined' ? '--resize-fit-width' : '--resize-fit-height',
            width !== 'undefined' ? width.toString() : height.toString(),
            '-i', // input
            '-', // use stdin
            '-o', // output
            '-' // use stdout
          ...
          proc.on('error', err => {
            console.log('gifsicle Error: ', (err || {}).message);
          });

          // stream original image into spawn process
          response.pipe(proc.stdin);

          // pipe spawn output to response
          const procStream = proc.stdout.pipe(res);

          // close stream after finish piping to client
          procStream.on('finish', () => {
            procStream.end();
          });
        ...
        // @see http://www.imagemagick.org/Usage/thumbnails/#cut
          ...
          /**
           * case where gif-compression feature is toggled off, we fallback to old logic
           * where gif are being served with their natural width/height without filters
           */

          // @see https://github.com/aheckmann/gm#constructor
          var im = imageMagick(response, 'image.' + extension);

          if (mimeType !== mime.getType('gif')) {
            ...
            // do not apply filters on gif as not all of them works properly
            if (filter && mimeType !== mime.getType('gif')) {
              if (filter === 'blur') {
                im.blur(0, 10);
...
function handleRequest(req, res, next, width, height, extension, filter, origin) {
  ...
  if (origin === 's3') {
    retrieveRemote(res, next, CDN_ORIGIN + req.params.url, width, height, extension, filter);
...
module.exports = function() {
  app.use(cors({ origin: /\.apester\.com$/ }));
  // Health route.
