...
function retrieveRemote(req, res, next, remote, width, height, extension) {
  // Use ** instead of / separator, good for some cases of encoding.
  ...
  var agent = options.protocol === 'http:' ? http : https
    ...
    , request = agent.get(options, function (response) {
        ...
        if ((response.statusCode === 301 || response.statusCode === 302) && response.headers['location']) {
          ...
          return retrieveRemote(req, res, next, url.format(redirect), weight, height, extension);
        }
...
module.exports = function () {
  app.get('/:url', function (req, res, next) {
    ...
    retrieveRemote(req, res, next, 'http://s3.amazonaws.com/qmerce-static-media/' + req.params.url, null, null, null);
  });
  ...
  app.get('/:url/:width/:height.:extension?', function (req, res, next) {
    ...
    retrieveRemote(req, res, next, 'http://s3.amazonaws.com/qmerce-static-media/' + req.params.url, width, height, extension);
  });

  app.get('/sharer/:interactionId', function (req, res, next) {

    // get url to process
    var url_to_process = 'http://renderer.qmerce.com/interaction/' + req.params.interactionId;
    ...
    cutiecapt.path = '/home/shushu/cutycapt/CutyCapt/CutyCapt';

    // Put images in public directory.
    var image_file_name = req.params.interactionId + ".png";
    var url = 'http://renderer.qmerce.com/interaction/' + req.params.interactionId;
    var image_path = "share/" + image_file_name;

    // First return a response.
    res.status(200).send();

    return cutiecapt.capture(url, image_path, function (err, file) {
      return s3UploadService(file).then(res.ok);
    });
  });
