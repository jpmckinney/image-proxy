  ...
  var agent = options.protocol === 'http:' ? http : https
    ...
    , request = agent.get(options, function (response) {
        ...
        if ((response.statusCode === 301 || response.statusCode === 302) && response.headers['location']) {
          ...
          return retrieveRemote(res, next, url.format(redirect), weight, height, extension, filter);
        }
        ...
        if (filter) {
          if (filter === "1") { // Sepia
            im.modulate(115, 0, 100).colorize(7, 21, 50);
          } else if (filter === "2") { // Blur
            im.blur(5000);
          } else if (filter === "3") { // Black and White
            im.colorspace("Gray");
          } else if (filter === "4") { // Saturation
            im.modulate(100, 200);
          }
        }
...
function handleRequest(req, res, next, width, height, extension, filter) {
  // Validate parameters.
  if (whitelist.length) {
...
module.exports = function () {
  app.get('/:url', function (req, res, next) {
    ...
    handleRequest(req, res, next, defaultWidth, null, null, null);
  });
  ...
  app.get('/:url/:width/:height.:extension?/:filter', function (req, res, next) {
    ...
    handleRequest(req, res, next, width, height, extension, filter);
  });
