...
var errLogPath = appRoot + config.logDir + config.logFile;
...
var logger = new (winston.Logger)({
  transports: [
    ...
    new (winston.transports.File)({ filename: errLogPath})
  ]
});
...
function retrieveRemote(res, next, remote, width, height, extension, filter) {
  ...
  request.setTimeout(delay, function () {
    ...
    res.status(504).send();
    return next({msg: 'Timeout', remote: remote});
  });
}
...
module.exports = function () {
  ...
  app.use(function(err, req, res, next) {
    if (err) {
      err.method    = req.methid;
      err.route     = req.route.path;
